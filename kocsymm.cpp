...
void permcube::set_edge_perm(cubepos &cp) const {
  ...
  for (int i = 0; i < 12; i++)
    if ((et_bits >> i) & 1) {
      ...
      cp.e[i] =
          cubepos::edge_val((3 & em_perm) + 4, cubepos::edge_ori(cp.e[i]));
      em_perm >>= 2;
    } else {
      cp.e[i] =
          cubepos::edge_val((3 & eb_perm) + 8, cubepos::edge_ori(cp.e[i]));
      eb_perm >>= 2;
    }
}
...
void permcube::set_corner_perm(cubepos &cp) const {
  ...
  for (int i = 0; i < 8; i++)
    if ((c8_4_bits >> i) & 1) {
      cp.c[i] =
          cubepos::corner_val((3 & ct_perm), cubepos::corner_ori(cp.c[i]));
      ct_perm >>= 2;
    } else {
      cp.c[i] =
          cubepos::corner_val((3 & cb_perm) + 4, cubepos::corner_ori(cp.c[i]));
      cb_perm >>= 2;
    }
}
...
#line 311 "kocsymm.w"
  ...
  for (int i = 0; i < CORNERSYMM; i++) {
    ...
    for (int mv = 0; mv < NMOVES; mv++) {
      ...
      if (i < EDGEOSYMM) edgeomove[i][mv] = kc2.eosymm;
...
#line 407 "kocsymm.w"
  ...
  if (c!=CORNERRSYMM) error("! bad cornersym result");
